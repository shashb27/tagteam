...
If you're logged into the Claude Code CLI on this machine, the Agent SDK rides your local credentials — no API key needed. Claude gets **read-only tools** (Read/Grep/Glob/web search) scoped to a small demo workspace, so it can do real work in the session. The server probes the SDK at startup and falls back automatically if it's unavailable.
